...
class Jobs extends Component {
  ...
  onGetJobDetails = async () => {
    ...
    const jwtToken = Cookies.get('jwt_token')
    const {checkboxInputs, radioInput, searchInput, locationCheckbox} =
      this.state
    const jobsApiUrl = `https://apis.ccbp.in/jobs?employment_type=${checkboxInputs}&minimum_package=${radioInput}&search=${searchInput}&location=${locationCheckbox}`
    const optionsJobs = {
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
  ...
  onGetLocationOption = event => {
    ...
    const locationNotInList = locationCheckbox.filter(
      eachItem => eachItem === event.target.id,
    )
    if (locationNotInList.length === 0) {
      ...
      const filteredLocationData = locationNotInList.filter(
        eachItem => eachItem !== event.target.id,
      )
      this.setState(
        {locationCheckbox: filteredLocationData},
        this.onGetJobDetails,
      )
    }
  }
